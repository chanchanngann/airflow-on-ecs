...
resource "aws_ecs_task_definition" "airflow_triggerer" {
  family                   = var.airflow_triggerer_task_name
  ...
  container_definitions = jsonencode([
    {
      ...
      command = ["triggerer"] # the image ENTRYPOINT automatically prepends "airflow"
  ...
  tags = merge(var.common_tags,
    {
      Name = var.airflow_triggerer_task_name
    }
  )
}
...
resource "aws_ecs_service" "airflow_triggerer" {
  ...
  network_configuration {
    ...
    security_groups  = [aws_security_group.ecs_airflow_triggerer_sg.id]
...
###################################
# ECS security group (triggerer)
###################################
resource "aws_security_group" "ecs_airflow_triggerer_sg" {
  name = "${var.name_prefix}-ecs-airflow-triggerer-sg"
...
# egress
# --------------------
resource "aws_vpc_security_group_egress_rule" "triggerer_allow_all_traffic_ipv4" {
  security_group_id = aws_security_group.ecs_airflow_triggerer_sg.id
